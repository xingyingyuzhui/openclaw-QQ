...
const sessionsPath = pathArg || `${process.env.OPENCLAW_HOME || `${process.env.HOME || ''}/.openclaw`}/agents/main/sessions/sessions.json`;
const OWNER_QQ = String(process.env.OPENCLAW_QQ_OWNER_ID || 'QQ_OWNER_ID').trim();
const OWNER_ROUTE = `user:${OWNER_QQ}`;
const OWNER_MAIN_SESSION_KEY = `agent:main:qq:user:${OWNER_QQ}`;

function isOwnerRoute(route) {
  return String(route || '').trim() === OWNER_ROUTE;
}
...
function toResidentKey(route) {
  if (isOwnerRoute(route)) return OWNER_MAIN_SESSION_KEY;
...
function parseRouteFromKey(key, obj) {
  let m = key.match(new RegExp(`^agent:qq-user-${OWNER_QQ}:qq:default:(user:${OWNER_QQ})$`));
  if (m) return m[1];
  m = key.match(new RegExp(`^agent:qq-user-${OWNER_QQ}:qq:user:(${OWNER_QQ})$`));
  if (m) return `user:${m[1]}`;
  m = key.match(new RegExp(`^agent:qq-user-${OWNER_QQ}:qq:(${OWNER_QQ})$`));
  if (m) return `user:${m[1]}`;
  m = key.match(new RegExp(`^agent:qq-user-${OWNER_QQ}:qq:(user:${OWNER_QQ})$`));
  if (m) return m[1];
  m = key.match(/^agent:(?:main|default):qq:default:(user:\d{5,12}|group:\d{5,12}|guild:[^:]+:[^:]+)$/);
  if (m) return m[1];
