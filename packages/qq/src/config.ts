...
export const QQConfigSchema = z.object({
  ...
  ownerUserId: z.string().optional().default("").describe("Optional QQ owner user id bound to main. Falls back to OPENCLAW_QQ_OWNER_ID."),
  ...
  sendRetryJitterRatio: z.number().optional().default(0.15).describe("Retry jitter ratio (0~1), e.g. 0.15 for ±15% jitter"),
  ...
  liteContextRoutes: z.array(z.string()).optional().default([]).describe("Routes that use lightweight prompt/context assembly to reduce token pressure. Supports exact route, prefix wildcard (user:*, group:*, guild:*), or global *"),
  blockStreaming: z.boolean().optional().default(false).describe("Enable live block streaming replies for QQ. Disabled by default so tool-using runs only deliver final replies/media"),
  ...
  napcatVersionPolicy: z.enum(["new-first-with-legacy-fallback", "legacy-first", "strict-new"]).optional().default("new-first-with-legacy-fallback").describe("NapCat action invocation policy"),
  napcatCapabilityProbeEnabled: z.boolean().optional().default(true).describe("Enable account-level NapCat action capability cache/probe"),
  napcatActionTimeoutMs: z.number().optional().default(5000).describe("Timeout in ms for one NapCat action request"),
  napcatActionMaxRetries: z.number().optional().default(1).describe("Retry attempts for transient NapCat action transport failures"),
  napcatActionRetryBaseDelayMs: z.number().optional().default(300).describe("Base delay in ms for NapCat action retry backoff"),
  napcatLogTraceEnabled: z.boolean().optional().default(true).describe("Enable structured NapCat action lifecycle trace"),
  ...
  proactiveDmRoute: z.string().optional().default("user:123456789").describe("Target route for proactive DM nudges"),
